classdef Simulator < handle
    ...
    properties
        ...
        binPath = '/home/kim/Documents/MATLAB/BiSMA/BiSMA_PRML/bin';
        ...
        % from source)
        libPath = '/home/kim/Documents/MATLAB/BiSMA/BiSMA_PRML/bin';
    ...
    methods
        ...
        function obj = Simulator(model, discretization, solverOptions)
            ...
            % Determine backend
            obj.useFileBackend = true; %(exist(['CadetMex.' mexext], 'file') ~= 3);
        ...
        function task = prepareSimulation(obj, skipValidation, skipSynch)
            ...
            for i = 1:length(obj.sensitivities)
                ...
                % indexMode determines how elements are indexed:
                % 0 (scalar), 1 (NCOMP values), 2 (NSEC / NREACT values), 
                % 3 (NCOMP * NSEC or NCOMP * NREACT values in component-major ordering)

                % Attention: SENS_COMP and SENS_SECTION are zero-based
                
                switch acc.indexMode
                    case 0
...
%                     end
        ...
        function result = run(obj, task)
            ...
            if obj.useFileBackend
                ...
                try
                    ...
                    fprintf('Simulation failed (file %s):\n%s', obj.exchangeFile, msgText);
                    N = length(obj.solutionTimes);
                    result.solution.SOLUTION_TIMES = obj.solutionTimes;
                    result.solution.SOLUTION_COLUMN_OUTLET_COMP_000 = ones(N,1) * 69.9744;
                    result.solution.SOLUTION_COLUMN_OUTLET_COMP_001 = ones(N,1) * 1.0;
                    result.solution.SOLUTION_COLUMN_OUTLET_COMP_002 = zeros(N,1);
                    
                    result.sensitivity.param_000.SENS_COLUMN_OUTLET_COMP_000 = ones(N,1) * 1E-18;
                    result.sensitivity.param_000.SENS_COLUMN_OUTLET_COMP_001 = ones(N,1) * 1E-18;
                    result.sensitivity.param_000.SENS_COLUMN_OUTLET_COMP_002 = ones(N,1) * 1E-18;
                    result.sensitivity.param_001.SENS_COLUMN_OUTLET_COMP_000 = ones(N,1) * 1E-18;
                    result.sensitivity.param_001.SENS_COLUMN_OUTLET_COMP_001 = ones(N,1) * 1E-18;
                    result.sensitivity.param_001.SENS_COLUMN_OUTLET_COMP_002 = ones(N,1) * 1E-18;
                    result.sensitivity.param_002.SENS_COLUMN_OUTLET_COMP_000 = ones(N,1) * 1E-18;
                    result.sensitivity.param_002.SENS_COLUMN_OUTLET_COMP_001 = ones(N,1) * 1E-18;
                    result.sensitivity.param_002.SENS_COLUMN_OUTLET_COMP_002 = ones(N,1) * 1E-18;
                    result.sensitivity.param_003.SENS_COLUMN_OUTLET_COMP_000 = ones(N,1) * 1E-18;
                    result.sensitivity.param_003.SENS_COLUMN_OUTLET_COMP_001 = ones(N,1) * 1E-18;
                    result.sensitivity.param_003.SENS_COLUMN_OUTLET_COMP_002 = ones(N,1) * 1E-18;
                    result.sensitivity.param_004.SENS_COLUMN_OUTLET_COMP_000 = ones(N,1) * 1E-18;
                    result.sensitivity.param_004.SENS_COLUMN_OUTLET_COMP_001 = ones(N,1) * 1E-18;
                    result.sensitivity.param_004.SENS_COLUMN_OUTLET_COMP_002 = ones(N,1) * 1E-18;
                    result.sensitivity.param_005.SENS_COLUMN_OUTLET_COMP_000 = ones(N,1) * 1E-18;
                    result.sensitivity.param_005.SENS_COLUMN_OUTLET_COMP_001 = ones(N,1) * 1E-18;
                    result.sensitivity.param_005.SENS_COLUMN_OUTLET_COMP_002 = ones(N,1) * 1E-18;
                    result.sensitivity.param_006.SENS_COLUMN_OUTLET_COMP_000 = ones(N,1) * 1E-18;
                    result.sensitivity.param_006.SENS_COLUMN_OUTLET_COMP_001 = ones(N,1) * 1E-18;
                    result.sensitivity.param_006.SENS_COLUMN_OUTLET_COMP_002 = ones(N,1) * 1E-18;
                    result.sensitivity.param_007.SENS_COLUMN_OUTLET_COMP_000 = ones(N,1) * 1E-18;
                    result.sensitivity.param_007.SENS_COLUMN_OUTLET_COMP_001 = ones(N,1) * 1E-18;
                    result.sensitivity.param_007.SENS_COLUMN_OUTLET_COMP_002 = ones(N,1) * 1E-18;
                    result.sensitivity.param_008.SENS_COLUMN_OUTLET_COMP_000 = ones(N,1) * 1E-18;
                    result.sensitivity.param_008.SENS_COLUMN_OUTLET_COMP_001 = ones(N,1) * 1E-18;
                    result.sensitivity.param_008.SENS_COLUMN_OUTLET_COMP_002 = ones(N,1) * 1E-18;
                    result.sensitivity.param_009.SENS_COLUMN_OUTLET_COMP_000 = ones(N,1) * 1E-18;
                    result.sensitivity.param_009.SENS_COLUMN_OUTLET_COMP_001 = ones(N,1) * 1E-18;
                    result.sensitivity.param_009.SENS_COLUMN_OUTLET_COMP_002 = ones(N,1) * 1E-18;
                end
            else
                % MEX
                data.input = task;
                try
                    ...
                    fprintf('CADET:simulationFailed \n%s', e.message);
                    N = length(obj.solutionTimes);
                    result.solution.SOLUTION_TIMES = obj.solutionTimes;
                    result.solution.SOLUTION_COLUMN_OUTLET_COMP_000 = ones(N,1) * 69.9744;
                    result.solution.SOLUTION_COLUMN_OUTLET_COMP_001 = ones(N,1) * 0.25;
                    result.solution.SOLUTION_COLUMN_OUTLET_COMP_002 = zeros(N,1);
                end
            end
    ...
    methods (Access = 'protected')
            ...
            for i = 1:length(obj.sensitivities)
                ...
                if useModel && ~isempty(obj.model)
                    ...
                    if isfield(curSens, 'SENS_SECTION') && ~((curSens.SENS_SECTION <= obj.model.nInletSections) || ...
                      (isfield(obj.model.reactionParameters, 'NREACT') && (curSens.SENS_SECTION <= obj.model.reactionParameters.NREACT)))
                        ok = false;
                        disp(['Error: In sensitivity ' num2str(i) ': Field SENS_SECTION has to be smaller than nInletSections (' num2str(obj.model.nInletSections) ') or reactionParameters.NREACT (' num2str(obj.model.reactionParameters.NREACT) ')']);
                    end
        ...
        function actualName = addSensitivityAccess(obj, name, compIdx, secIdx)
            ...
            [result, path, indexMode] = obj.model.hasParameter(name);

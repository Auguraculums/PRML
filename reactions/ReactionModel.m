...
classdef ReactionModel < handle
    %REACTIONMODEL Base class for all reaction models
    %
    % Copyright: © 2008-2015 Eric von Lieres, Joel Andersson, Andreas Püttmann, Sebastian Schnittert, Samuel Leweke
    %            See the license note at the end of the file.
    
    properties
        name;       % Name of the reaction model as given in the file format specification
        parameters; % A cell array of structs which define parameters of this reaction model
    end
    
    methods
        
        function obj = ReactionModel()
            %REACTIONMODEL Constructs a ReactionModel object
            obj.name = 'REACTION';
            obj.parameters = [];
        end
        ...
        function [result, paramName, indexMode] = hasParameter(obj, paramName)
            ...
            result = ~isempty(idx);
            if result
                par = obj.parameters{idx(1)};
                paramName = par.name;
                
                if par.perComponent && par.perReaction
                    indexMode = 3;
                elseif par.perComponent && ~par.perReaction
                    indexMode = 1;
                elseif ~par.perComponent && par.perReaction
                    indexMode = 2;
                else
    ...
    methods (Access = 'protected', Static)
        ...
        function ok = checkNonnegativeMatrix(val, sz, name)
            % checkNonnegativeMatrix Checks whether the given matrix is nonnegative and has a certain size
            %
            % Parameters:
            %   - val: Vector to check
            %   - sz: Valid size (rows x cols)
            ...
            if any(size(val) ~= sz)
                ok = false;
                disp(['Error: ' name ' has to be a matrix of size ' num2str(sz(1)) ' x ' num2str(sz(2))]);
            end
            if any(val(:) < 0.0)
                ok = false;
        ...
        function ok = checkVectorSize(val, len, name)
            % checkVectorSize Checks whether the given vector has a certain length
            %
            % Parameters:
            %   - val: Vector to check
            %   - len: Valid length
        ...
        function param = createParameter(name, displayName, perComponent, perReaction)
            ...
            if (nargin <= 3) || isempty(perReaction)
                perReaction = true;
            end

...
classdef ModelGRM < handle
    ...
    properties
        ...
        reactionParameters;     % Struct with parameters of the reaction model
    ...
    properties (Dependent = true)
        ...
        reactionModel;              % Object of the used reaction model class
    end
    ...
    properties (Access = 'protected')
        ...
        reactionHandle;    % Storage for the actual object of the used ReactionModel class
    end
    ...
    methods
        ...
        function obj = ModelGRM()
            ...
            obj.reactionHandle = DummyReaction();
            obj.bindingHandle = [];
        ...
        function ok = checkValues(obj)
            ...
            % Reaction model
            if isempty(obj.reactionHandle)
                ok = false;
                disp('Error: Binding model is unset');
            end

            params = obj.reactionHandle.parameters;
            for i = 1:length(params)
                curPar = params{i};
                if ~isfield(obj.reactionParameters, curPar.name)
                    ok = false;
                    disp(['Error: reactionParameters has to contain the field "' curPar.name '" when reaction model ' obj.reactionHandle.name ' is chosen']);
                end
            end

            if ~obj.reactionHandle.checkParameters(obj.reactionParameters, obj.nComponents)
                ok = false;
                disp(['Error: reactionParameters are not valid for reaction model ' obj.reactionHandle.name]);
            end
        ...
        function val = get.reactionModel(obj)
            val = obj.reactionHandle;
        end
        
        function set.reactionModel(obj, newReaction)
            obj.reactionParameters = [];
            if ~isempty(newReaction) && isobject(newReaction)
                % Take parameters from reaction model
                obj.reactionHandle = newReaction;
                ...
                for i = 1:length(params)
                    ...
                    obj.reactionParameters.(par.name) = value;
                end
            else
                obj.reactionHandle = DummyReaction();
            end
        ...
        function synchToStruct(obj)
            ...
            params = obj.bindingHandle.parameters;
            convParams = obj.bindingHandle.convertParametersToStruct(obj.bindingParameters);
            for i = 1:length(params)
                par = params{i};
                obj.input.model.adsorption.(par.name) = convParams.(par.name);
            end

            obj.input.model.REACTION_TYPE = obj.reactionHandle.name;
            
            % Copy reaction parameters
            params = obj.reactionHandle.parameters;
            convParams = obj.reactionHandle.convertParametersToStruct(obj.reactionParameters);
            for i = 1:length(params)
                par = params{i};
                obj.input.model.reactions.(par.name) = convParams.(par.name);
            end
        ...
        function synchFromStruct(obj, input)
            ...
            % Try to find the reaction model by looking through all classes in the "reactions" folder
            obj.reactionHandle = [];
            pathToReactions = fullfile(fileparts(mfilename('fullpath')), 'reactions');
            classes = dir(fullfile(pathToReactions, '*.m'));
            for i = 1:length(classes)
                % Create instance of class
                [~, name, ~] = fileparts(classes{i});
                instance = eval([name '();']);

                if strcmpi(instance.name, obj.input.model.REACTION_TYPE)
                    obj.reactionHandle = instance;
                    break;
                end
            end

            if isempty(obj.reactionHandle)
                warning('CADET:reactionNotFound', 'Could not find reaction model class named %s', obj.input.model.REACTION_TYPE);
            end
            ...
            obj.bindingParameters = obj.bindingHandle.convertParametersToProperty(obj.bindingParameters);
            
            % Copy reaction model parameters
            params = obj.reactionHandle.parameters;
            for i = 1:length(params)
                par = params{i};
                ...
                if isfield(obj.input.model.reactions, par.name)
                    % Set given value
                    obj.reactionParameters.(par.name) = obj.input.model.reactions.(par.name);
                else
                    % Set default
                    obj.reactionParameters.(par.name) = value;
                end
            end
            obj.reactionParameters = obj.reactionHandle.convertParametersToProperty(obj.reactionParameters);
        ...
        function [result, path, indexMode] = hasParameter(obj, paramName)
            ...
            %   - indexMode: Determines how to index the parameters. Valid
            %       values are 0 (scalar), 1 (NCOMP values), 2 (NSEC values),
            %       3 (NCOMP * NSEC values in component-major ordering)
                        
            idxModel = find(cellfun(@checkParamExistence, obj.trackingInfo));
            if ~isempty(idxModel)
                result = true;
                par = obj.trackingInfo{idxModel(1)};
                path = par.structName;
                ...
                if (par.secDepMode == 1) && (length(obj.(par.propName)) == obj.nInletSections)
                    indexMode = 2;
                elseif (par.secDepMode == 2) && (length(obj.(par.propName)) == obj.nInletSections * obj.nComponents)
                    indexMode = 3;
                end
            ...
            if ~isempty(obj.bindingHandle)
                [result, paramNameOut, indexMode] = obj.bindingHandle.hasParameter(paramName);
                if result
                    path = ['model.adsorption.' paramNameOut];
                    return;
                end
            end

            if ~isempty(obj.reactionHandle)
                [result, paramNameOut, indexMode] = obj.reactionHandle.hasParameter(paramName);
                if result
                    path = ['model.reactions.' paramNameOut];
                    return;
                end
            end
    ...
    methods (Access = 'protected')
        ...
        function createTrackingInfo(obj)
            ...
            obj.addTracking('nComponents', 'model.NCOMP', @(x) int32(x), @(x) double(x), true);
            ...
            obj.addTracking('kineticBindingModel', 'model.adsorption.IS_KINETIC', @(x) int32(x), @(x) x == 1, true);
            obj.addTracking('reactionParameters', 'model.reactions', [], [], true);
            
            % Transport
            obj.addTracking('dispersionColumn', 'model.COL_DISPERSION', [], [], true, 1);
            obj.addTracking('interstitialVelocity', 'model.VELOCITY', [], [], true, 1);

            obj.addTracking('porosityColumn', 'model.COL_POROSITY', [], [], true);
            obj.addTracking('porosityParticle', 'model.PAR_POROSITY', [], [], true);

            obj.addTracking('filmDiffusion', 'model.FILM_DIFFUSION', [], [], false, 2);
            obj.addTracking('diffusionParticle', 'model.PAR_DIFFUSION', [], [], false, 2);
            obj.addTracking('diffusionParticleSurface', 'model.PAR_SURFDIFFUSION', [], [], false, 2);

            % Geometry
            obj.addTracking('columnLength', 'model.COL_LENGTH', [], [], true);
            obj.addTracking('particleRadius', 'model.PAR_RADIUS', [], [], true);

            % Inlet
            obj.addTracking('nInletSections', 'model.inlet.NSEC', @(x) int32(x), @(x) double(x), true);
            ...
            obj.addTracking('extFuncVelocity', 'model.external.EXT_VELOCITY', [], [], true, 0, true);
            obj.addTracking('extFuncProfile', 'model.external.EXT_PROFILE', [], [], false, 0, true);
            obj.addTracking('extFuncDelta', 'model.external.EXT_PROF_DELTA', [], [], false, 0, true);
        end

        function addTracking(obj, propName, structName, convToStruct, convToProp, isScalar, secDepMode, optional)
            ...
            %   - isScalar: Optional. Determines whether this parameter is
            %       scalar (neglecting section dependency). Defaults to
            %       false.
            %   - secDepMode: Optional. Determines section dependence of
            %       parameters. 0 independent, 1 scalar dependent (NSEC 
            %       values), 2 vector dependent (NSEC * NCOMP values).
            %       Defaults to 0.
            ...
            if nargin <= 5 || isempty(isScalar)
                info.isScalar = false;
            else
                info.isScalar = isScalar;
            end
            
            if nargin <= 6 || isempty(secDepMode)
                info.secDepMode = 0;
            else
                info.secDepMode = secDepMode;
            end
            
            if nargin <= 7 || isempty(optional)
                info.optional = false;
            else
